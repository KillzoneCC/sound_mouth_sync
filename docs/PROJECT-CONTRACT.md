...
- **Железо рта:** модуль OLED SSD1306 должен отвечать на **0x3D**; на плате зона **IIC ADDRESS SELECT** (резистор **0x78** = 7-bit **0x3C**, **0x7A** = **0x3D**). Без переноса перемычки на рту при двух модулях возможен дубль SSID/IP на втором экране. См. `doc/images/oled_i2c_address_select_example.png`, `doc/VISIO_SCHEME_OLED_JUMPER.md`.
...
| v0.1.3 | 2026-04-02 | Детализация | Документация: **IIC ADDRESS SELECT** (0x78/0x7A ↔ 0x3C/0x3D), фото `doc/images/oled_i2c_address_select_example.png`, `doc/VISIO_SCHEME_OLED_JUMPER.md`; обновлены README, ресурсы, ROADMAP | Активно |
