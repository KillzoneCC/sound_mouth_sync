...
Осциллограмма попадает на экран только если **`mouth_display_node`** рисует на **правильном** физическом адресе OLED рта (**0x3D**); это задаётся **перемычкой IIC ADDRESS SELECT** на модуле (см. [ARCHITECTURE.md](ARCHITECTURE.md), [README.md](../README.md)). Проблема «волна в топиках есть, на экране чужая картинка» сначала проверяйте I²C, не только аудио.
