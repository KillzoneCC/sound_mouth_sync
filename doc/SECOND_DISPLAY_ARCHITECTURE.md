...
Оба модуля висят на **одной** шине I²C. Контроллер SSD1306 **слушает тот адрес, который задан перемычкой на плате** — на многих модулях это блок **IIC ADDRESS SELECT** (позиция припаянного резистора): метки **0x78** / **0x7A** на PCB соответствуют **7-bit** адресам **0x3C** / **0x3D** соответственно. Адрес **не** «назначается» ROS при каждой загрузке; ПО только обращается к выбранному номеру. Если физически **два** модуля с одинаковой позицией (**оба 0x3C**), то **любая** транзакция на **0x3C** выполняется **обоими** чипами — на «рту» будет та же картинка, что и на системном экране. ПО **не** направляет пиксели статистики на **0x3D**: `oled_display.py` при активной разработке **запрещает** целевой адрес **0x3D** для статистики (`RuntimeError` при `AINEX_INFO_OLED_I2C_ADDR=0x3D`). Фото примера и памятка для схемы Visio: `doc/images/oled_i2c_address_select_example.png`, [VISIO_SCHEME_OLED_JUMPER.md](VISIO_SCHEME_OLED_JUMPER.md).
